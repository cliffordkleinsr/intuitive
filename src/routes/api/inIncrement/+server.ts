...
export async function GET() {
	const identifier = 'signin_api_call_counter';
	...
	try {
		...
		const lastCalled = await redis.get<string>('last_called_signin');
		const lastCalledAt = lastCalled ?? 'Never';

		// Update last called timestamp
		await redis.set('last_called_signin', new Date().toISOString());
