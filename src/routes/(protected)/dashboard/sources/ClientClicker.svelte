<script lang="ts">
	...
	import CustomProgress from '$lib/components/ui/progress/custom-progress.svelte';
	...
	let { series, total_clients } = $props();
	...
	// $inspect(week)
	let value = $state((total_clients / 250) * 100);
</script>
...
<Dialog.Root>
	...
	<Dialog.Content class="w-full max-w-5xl">
		<Dialog.Title class="m-1 flex gap-2 rounded-md bg-secondary p-10">
			<div class="mr-auto">
				<h1 class="text-2xl font-semibold">Client Analytics</h1>
