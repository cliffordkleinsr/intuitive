<script lang="ts">
	...
	import { counties, employments, incomes } from '$lib/json';
...
<div class="flex h-[85vh] items-center justify-center p-4">
	<Card.Root>
		<Card.Header>
		...
		<Card.Footer>
			{#if $delayed}
				<Button class="flex gap-2" disabled={$delayed}>
					<span
						class="inline-block size-4 w-full animate-spin rounded-full border-[3px] border-current border-t-transparent text-white"
						role="status"
						aria-label="loading"
					></span>
					Loading...
			...
			{:else}
				<Form.Button variant="black" class="w-full">Update Profile And Change Password</Form.Button>
			{/if}
		</Card.Footer>
	</Card.Root>
</div>
